...
ivbds <- function(y, a, z, x, nsplits = 2,
  ...
  nulhat <- vl1hat - vl0hat
  nuuhat <- vu1hat - vu0hat
  gamhat <- la1hat - la0hat

  latehat <- mean(ifvals.out) / mean(ifvals.trt)
  bl.ate <- mean(ifvals.atel)
  bu.ate <- mean(ifvals.ateu)
  bl.hq <- mean(ifvals.hql.num) / mean(ifvals.trt)
  bu.hq <- mean(ifvals.hqu.num) / mean(ifvals.trt)
